"""
ORIGINAL AUTHOR: Decision, Action, and Neural Computation lab (Maciek Szul, James Bonaiuto)
ORIGINAL REPOSITORY: https://github.com/danclab/burst_detection
MODIFICATIONS BY: Juliette Leclerc

DESCRIPTION OF CHANGES:
This script is a modified version of the original burst detection algorithm. 
The script has been modified to not substract the averaged FOOOF aperiodic fit over all epochs
but to substract the averaged FOOOF aperiodic fit to a specific epoch over selected number of epoch around it.
"""
...
def extract_bursts(raw_trials, tf, times, search_freqs, band_lims, aperiodic_spectrum, sfreq, w_size=.26):
    ...
    for t_idx, tr_tf in tqdm(list(enumerate(tf)), total=len(tf), desc="Burst Detection"):
        
        '''START MODIFIED FROM ORIGINAL'''
        # Extract trial-specific aperiodic spectrum
        aperiodic_spectrum_trial = aperiodic_spectrum[t_idx, ...]

        # Regress out ERF from the current trial
        slope, intercept, r, p, se = linregress(erf, raw_trials[t_idx, :])
        raw_trial = raw_trials[t_idx, :] - (intercept + slope * erf)

        # Run burst extraction for this trial using its specific aperiodic spectrum
        trial_bursts = extract_bursts_single_trial(
            raw_trial,
            tr_tf,
            times,
            search_freqs,
            band_lims,
            aperiodic_spectrum_trial,
            sfreq,
            w_size=w_size
        )
